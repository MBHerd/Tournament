    ...
    <>
      ...
      <section className="grid" style={{ marginTop: 16 }}>
        {schemaGroups.map((group) => (
          <article className="card" key={group.name}>
            ...
            <div className="badges">{group.tables.map((table) => <span className="badge blue" key={table}>{table}</span>)}</div>
          </article>
        ))}
      </section>
      ...
      <section className="grid two">
        <section className="panel">
          ...
          <div className="panel-body table-wrap"><table><thead><tr><th>Area</th><th>Statuses</th></tr></thead><tbody>{Object.entries(statusCatalog).map(([area, statuses]) => <tr key={area}><td>{area}</td><td>{statuses.join(', ')}</td></tr>)}</tbody></table></div>
        </section>
        <section className="panel">
          ...
          <div className="panel-body"><div className="badges">{publicHiddenFields.map((field) => <span className="badge red" key={field}>{field}</span>)}</div></div>
        </section>
      </section>
      ...
      <section className="panel">
        ...
        <div className="panel-body"><div className="grid">{dataSeparationRules.map((rule) => <div className="card" key={rule}><h3>Rule</h3><p>{rule}</p></div>)}</div></div>
      </section>
    </>
  );
}
