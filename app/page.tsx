    ...
    <>
      ...
      <section className="grid two">
        <Panel title="Foundation Modules" subtitle="Clean boundaries for the modules named in the prompt">
          <div className="grid">
            {demo.modules.map((module) => <div className="card" key={module}><h3>{module}</h3><p>Domain boundary created for future server actions and database screens.</p></div>)}
          </div>
        </Panel>
      ...
      <Panel title="Demo Mini Tournament" subtitle="Eight teams, two pools, generated standings, and four-team bracket seed">
        ...
        <div className="badges">
          {bracket.seeds.map((seed) => <span className="badge blue" key={seed.seed}>Seed {seed.seed}: {seed.teamName}</span>)}
        </div>
      </Panel>
    </>
  );
